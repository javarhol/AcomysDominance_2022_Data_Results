...
```{r}
week_rank <- read.csv("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/dscorebweek_edited.csv")
week_rank$Cage <- as.factor(week_rank$Cage)
sex_id <- read_excel("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/AcomysDomRegen_Inventory.xlsx", sheet = "Sheet3")
...
dumcage <- read.csv("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/dscorebyanimal_edited.csv")
...
cumm_dom <- read.csv("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/dscorebyanimal_edited.csv")
...
rank_stab <- week_rank_stab %>% 
  ...
  distinct()
sex_id2 <- read_excel("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/AcomysDomRegen_Inventory.xlsx", sheet = "Sheet1")
...
rank_stab$stab_status <- factor(rank_stab$stab_status, levels=c("Stable", "Unstable", "Unclear"))

agestab_plot <- ggplot(rank_stab, aes(stab_status, AgeAtTest, color=stab_status, label=DumCage))+
  geom_jitter(size=4) +
  scale_color_npg()+
...
```
```{r}
m.agestab <- lmer(as.numeric(stab_status) ~ (1|Cage) + AgeAtTest, data=rank_stab)
...
ggsave("/Users/justinvarholick/Documents/GitHub/AcomysDominance_2022_Data_Results/R generated figures/Fig2_stab_plots.tiff", figx, units="in", width = 6.5, height = 4, device='tiff', dpi=300)
...
rank_stab <- week_rank_stab %>% 
  ...
  rename("AnimalID"=Opponent.1) %>% 
  distinct()
write.csv(rank_stab, "/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/stabilitybycage.csv")
```
