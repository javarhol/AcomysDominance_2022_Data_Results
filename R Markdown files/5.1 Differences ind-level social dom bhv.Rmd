...
#Test, sex
```{r}
m.sex.dom <- glm(n~Behavior + Sex + Behavior*Sex, data=sex_dyads_dom)
...
#Test, sex sub
```{r}
m.sex.sub <- glm(n~Behavior + Sex + Behavior*Sex, data=sex_dyads_sub)
anova(m.sex.sub, test="F")
summary(m.sex.sub)
```
...
#test, sex domsub
```{r}
m.sex.domsub <- lmer(n ~ (1|Cage) + group + Sex + group*Sex, data=sex_dyads_domsub, REML=F, control=lmerControl(optimizer="bobyqa"))
...
#Test, grpsize
```{r}
m.grp.dom <- glm(n~Behavior + as.factor(GrpSize) + Behavior*as.factor(GrpSize), data=males_dom)
...
#Test, grpsize sub
```{r}
m.grp.sub <- glm(n ~  Behavior + as.factor(GrpSize) + Behavior*as.factor(GrpSize), data=males_sub)
...
```
```{r}
m.males.domsub <- lmer(n ~ (1|Cage) + group + as.factor(GrpSize) + group*as.factor(GrpSize), data=males_domsub, REML=F, control=lmerControl(optimizer="bobyqa"))
...
```{r grouped plot}
dom_sex_plot <- ggplot(sex_dyads_dom, aes(Behavior, n, fill=Sex))+
  geom_boxplot()+
  scale_fill_npg()+
  ylab("Total freq.") + xlab("")

domsub_sex_plot <- ggplot(sex_dyads_domsub, aes(group, n, fill=Sex, color=Sex, group=Cage)) +
  geom_point(size=3) + geom_line(size=1) +
  scale_fill_npg() + scale_color_npg() +
  scale_x_discrete(labels=c("Dominant Offensive", "Subordinate Defensive")) +
  ylab("Total freq.") + xlab("")

males_plot <- ggplot(males_dom, aes(Behavior, n, fill=as.factor(GrpSize)))+
  geom_boxplot()+
  scale_fill_npg()+
  ylab("Total freq.") + xlab("") +
  guides(fill=guide_legend("Group Size"))

fig3 <- plot_grid(
  dom_sex_plot, domsub_sex_plot, males_plot,
