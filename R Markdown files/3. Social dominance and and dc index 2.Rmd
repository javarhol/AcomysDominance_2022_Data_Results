---
title: "3. Social dominance and steepness 2"
author: "Justin Varholick"
date: '2022-06-15'
output: html_document
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
#plots
library(tidyverse)
library(ggplot2)
library(ggbeeswarm)
library(ggthemr) #devtools::install_github('Mikata-Project/ggthemr')
library(ggsci)
library(ggpubr)
library(ggstance)
library(cowplot)
library(readxl)
library(patchwork)

ggthemr('flat')
```

## Social dominance status data
```{r}
ind_rank_wins <- read.csv("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/winsbyanimal_edited.csv")
ind_rank_wins <- ind_rank_wins[,2:6]
ind_rank_wins <- rename(ind_rank_wins, "Cage"="Cage.x")
sex_id <- read_excel("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/AcomysDomRegen_Inventory.xlsx", sheet = "Sheet3")
sex_id$Cage <- as.factor(sex_id$Cage)
ind_rank_wins$Cage <- as.factor(ind_rank_wins$Cage)
ind_rank_wins <- left_join(ind_rank_wins, sex_id, by="Cage")

ind_ds <- read.csv("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/dscorebyanimal_edited.csv")
ind_ds$Cage <- as.factor(ind_ds$Cage)
ind_ds <- left_join(ind_ds, ind_rank_wins, by=c("AnimalID"="Opponent.1"))
ind_ds$dom_status_ds <- as.factor(ind_ds$dom_status_ds)

dc_cage <- read.csv("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/dcbycage_edited.csv")
cages <- ind_ds %>% 
  select(Cage.x, DumCage, Sex, GrpSize, Age, Trt) %>% 
  distinct() %>% 
  rename(Cage=Cage.x)
dc_cage$Cage <- as.factor(dc_cage$Cage)
dc_cage <- left_join(dc_cage, cages, by="Cage")

stp <- read.csv("/Volumes/GoogleDrive-105793286447324059497/My Drive/UF/Data_AcomysDominance/Data for Publication/steepbycage_edited.csv")
stp$Cage <- as.factor(stp$Cage)
stp <- left_join(stp, ind_ds, by=c("Cage"="Cage.x"))
stp <- stp %>% 
  mutate(norm_ds=ds+(GrpSize*(GrpSize-1)/2))
```
##Social dominance graphs
```{r, david scores}
ds_plot <- ggplot(ind_ds, aes(as.factor(DumCage), ds, color=dom_status_ds, shape=dom_status_ds))+
  geom_point(size=3, alpha=1, position=position_dodge(width=0.5))+
  scale_color_npg()+
  xlab("") + ylab("David's Score")+
  guides(color=guide_legend("Dominance Status"), shape=guide_legend("Dominance Status"))
ds_plot
```
```{r, dc index}

dc_cage$Trt <- factor(dc_cage$Trt, levels=c("YFD", "YMD", "YMT", "OFD"))

dc_plot <- ggplot(dc_cage, aes(as.factor(DumCage), dc, fill=Trt))+
  geom_bar(stat="identity")+
  scale_y_continuous(expand=c(0,0))+
  scale_fill_npg()+
  ylab("DC index") + xlab("")+
  guides(fill=guide_legend("Housing Condition"))
dc_plot
```
```{r grouped plot}
fig1 <- plot_grid(
  ds_plot, dc_plot,
  labels="AUTO", ncol=2
)

ggsave("/Users/justinvarholick/Documents/GitHub/AcomysDominance_2022_Data_Results/R generated figures/Fig1_plot.tiff", fig1, units="in", width = 6.5, height = 2.16, device='tiff', dpi=300)

fig1
```

##Analyses were done by hand in reference to Siegel and Castellan 1988, "Nonparametric Statistics for the Behavioral Sciences" pg 130