...
sexdyads_ag$Behavior <- str_wrap(sexdyads_ag$Behavior, width=10)


sexplot <- ggplot(sexdyads_ag, aes(Behavior, n, fill=Sex))+
  geom_boxplot()+
  xlab("")+
  scale_fill_npg()
sexplot
...
grpsize_ag$Behavior <- str_wrap(grpsize_ag$Behavior, width=10)

grpsize_plot <- ggplot(grpsize_ag, aes(Behavior, n, fill=as.factor(GrpSize)))+
  geom_boxplot()+
  xlab("")+
  scale_fill_npg()+
  labs(fill="Group \nSize")
...
age_ag$Behavior <- str_wrap(age_ag$Behavior, width=10)
...
```{r grouped plot}
fig1 <- ggdraw() + 
  draw_plot(sexplot, x=0, y=.66, width=1, height=.33) +
  draw_plot(grpsize_plot, x=0, y=.33, width=1, height=.33)+
  draw_plot(age_plot, x=0, y=0, width=1, height=.33)+
  draw_plot_label(label=c("A","B","C"), size=12, x=c(0,0,0), y=c(1,.66,.33))

ggsave("/Users/justinvarholick/Documents/GitHub/AcomysDominance_2022_Data_Results/R generated figures/Fig1_ind_ag_bhv.tiff", fig1, units="cm", width = 17, height = 14, device='tiff', dpi=300)
