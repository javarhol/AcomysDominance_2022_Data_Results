...
status_plot_sex <- ggplot(sexdyads, aes(as.factor(DumCage), ds, color=dom_status_ds, shape=dom_status_ds)) +
...
status_plot_grp <- ggplot(grpsize_male, aes(as.factor(DumCage), ds, color=dom_status_ds, shape=dom_status_ds)) +
...
status_plot_age <- ggplot(age_female, aes(as.factor(DumCage), ds, color=dom_status_ds, shape=dom_status_ds)) +
  geom_point(size=3, alpha=1) +
  xlab("")+ ylab("David's Score")+
...
```{r grouped plot}
fig1 <- plot_grid(
  status_plot_sex, status_plot_grp, status_plot_age,
  labels = "AUTO", ncol = 1
)

ggsave("/Users/justinvarholick/Documents/GitHub/AcomysDominance_2022_Data_Results/R generated figures/Fig1_status_plots.tiff", fig1, units="cm", width = 5, height = 15, device='tiff', dpi=300)
