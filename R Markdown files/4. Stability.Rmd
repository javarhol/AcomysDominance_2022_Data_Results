...
river_plot_sex <- ggplot(sexdyads, aes(axis1=Week1, axis2=Week2, axis3=Week3))+
...
```{r}
grpsize_male <- river_data %>% 
  filter(Sex=="M")

river_plot_grp <- ggplot(grpsize_male, aes(axis1=Week1, axis2=Week2, axis3=Week3))+
  geom_alluvium(aes(fill=stab_status))+
  geom_stratum()+
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits=c("1", "2", "3"), expand=c(.05,.05))+
...
```
```{r}
age_female <- river_data %>% 
  filter(Sex=="F")

river_plot_age <- ggplot(age_female, aes(axis1=Week1, axis2=Week2, axis3=Week3))+
  geom_alluvium(aes(fill=stab_status))+
  geom_stratum()+
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits=c("1", "2", "3"), expand=c(.05,.05))+
...
```{r grouped plot}
fig2 <- plot_grid(
  river_plot_sex, river_plot_grp, river_plot_age,
  labels = "AUTO", ncol = 1
)
