...
sexdyads_zeros <- left_join(zeromatrix, sexdyads, by=(c("Opponent.1", "Sex", "dom_status_ds", "Cage", "Behavior")))
...
sexdyads_ag1 <- sexdyads_ag %>% 
  filter(Behavior=="Induced Flee") %>% 
  mutate(Behavior="Induced \nFlee")

sexdyads_ag2 <- sexdyads_ag %>% 
  filter(Behavior!="Induced Flee")

sexdyads_ag <- rbind(sexdyads_ag1, sexdyads_ag2)

sexdyads_ag$Behavior <- factor(sexdyads_ag$Behavior, levels=c("Chasing", "Mounting", "Attacking", "Stealing", "Fleeing", "Induced \nFlee", "Freeze", "Unseen", "Inactive"))
```


```{r plot sex effects}
#subset out dominants
sexdyads_dom <- filter(sexdyads_ag, dom_status_ds=="Dominant")

off <- c("Chasing", "Attacking", "Mounting", "Stealing")
def <- c("Fleeing", "Induced Flee", "Freeze")

sexplot <- ggplot(sexdyads_dom, aes(Behavior, n, fill=Sex))+
  geom_boxplot()+
  xlab("")+
  scale_fill_npg()+
...
sexdyads_dom_off <- filter(sexdyads_dom, Behavior %in% off)

dom_sex_mdl_off <- lm(n~Behavior + Sex + Behavior*Sex, data=sexdyads_dom_off)

anova(dom_sex_mdl_off)
summary(dom_sex_mdl_off)


```
```{r}
#subset out subordinates
sexdyads_sub <- filter(sexdyads_ag, dom_status_ds=="Subordinate")

off <- c("Chasing", "Attacking", "Mounting", "Stealing")
def <- c("Fleeing", "Induced Flee", "Freeze")

sexplot <- ggplot(sexdyads_sub, aes(Behavior, n, fill=Sex))+
  geom_boxplot()+
  xlab("")+
  scale_fill_npg()+
  theme(axis.text.x=element_blank())
...
sexdyads_sub_def <- filter(sexdyads_sub, Behavior %in% def)

dom_sex_mdl_def <- lm(n~Behavior + Sex + Behavior*Sex, data=sexdyads_sub_def)

anova(dom_sex_mdl_def)
summary(dom_sex_mdl_def)
