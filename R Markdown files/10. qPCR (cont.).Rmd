...
```{r}
Cyp11a1_m_f <- lmer(GeneExpRat ~  (1|Cage.x) + AgeAtTest + Sex + GrpSize + ds, filter(qpcr_data, target.x=="Cyp11a1"), REML=TRUE)
Star_m_f <- lmer(GeneExpRat ~  (1|Cage.x) + AgeAtTest + Sex + GrpSize + ds, filter(qpcr_data, target.x=="StAR"), REML=TRUE)
Nr5a1_m_f <- lmer(GeneExpRat ~  (1|Cage.x) + AgeAtTest + Sex + GrpSize + ds, filter(qpcr_data, target.x=="Nrb01"), REML=TRUE)
Nrb01_m_f <- lmer(GeneExpRat ~  (1|Cage.x) + AgeAtTest + Sex + GrpSize + ds, filter(qpcr_data, target.x=="Nr5a1"), REML=TRUE)
Cyp11b1_m_f <- lmer(GeneExpRat ~  (1|Cage.x) + AgeAtTest + Sex + GrpSize + ds, filter(qpcr_data, target.x=="Cyp11b1"), REML=TRUE)
...
ggplot(cyp11b1, aes(target.x, GeneExpRat, color=Sex, group=Sex))+
